...
const create_RpcClient = function () {
    ...
    const getM3u8UrlsByActiveTabId = async activeTabId => {
        ...
        console.log(`getM3u8UrlsByActiveTabId => tabInfo.m3u8_urls.length: ${tabInfo?.m3u8_urls?.length || '?'}`);
...
const create_WorkInfoType = stored => {
    ...
    deleteUrlFromTab = async (tabId, m3u8_url, save2Storage) => {
        if ((tabId !== undefined) && m3u8_url) {
            ...
            if (o?.m3u8_urls) {
                ...
                if (i !== -1) {
                    ...
                    await setUrlsCountText(tabId);
                    if (save2Storage !== false) await save2Storage();
                }
            }
        }
    },
