...
using _Expression_ = System.Linq.Expressions.Expression;
...
namespace System.Windows.Forms.Taskbar
{
    ...
    public sealed class PropVariant : IDisposable
    {
        ...
        private static Dictionary<Type, Action<PropVariant, Array, uint>> GenerateVectorActions()
        {
            var cache = new Dictionary<Type, Action<PropVariant, Array, uint>>
            {
...

                {
                    ...
                    (pv, array, i) =>
                    {
                        PropVariantNativeMethods.PropVariantGetFileTimeElem(pv, i, out _FILETIME_ val);
        ...
        private static Func<object, PropVariant> GetDynamicConstructor( Type type )
        {
            lock ( _padlock )
            {
                ...
                if ( !_cache.TryGetValue( type, out var action ) )
                {
                    ...
                    var constructor = typeof(PropVariant).GetConstructor( [ type ] );
                    ...
                    else // if the method was found, create an expression to call it.
                    {
                        ...
                        action = _Expression_.Lambda<Func<object, PropVariant>>( create, arg ).Compile();
        ...
        private static long GetFileTimeAsLong( ref _FILETIME_ val ) => (((long) val.dwHighDateTime) << 32) + val.dwLowDateTime;

        private static _FILETIME_ DateTimeToFileTime( DateTime value )
        {
            long hFT = value.ToFileTime();
            var ft = new _FILETIME_()
            {
        ...
        public override string ToString() => $"{Value}: {VarType}";
    }
}