...
using static m3u8.download.manager.ipc.BrowserIPC;
...
namespace m3u8.download.manager
{
    ...
    internal static class Extensions
    {
            ...
            try
            {
                ...
                if ( !text.IsNullOrEmpty() )
                {
                    ...
                    foreach ( var a in array )
                    {
                        ...
                        if ( !ExtensionRequestHeader.Try2Dict( requestHeaders, out var dict ) || !dict.AnyEx() )
                        {
                            requestHeaders = null;
                        }
        ...
        {
            var txt = string.Join( "\r\n", rows.Select( r => r.RequestHeaders.AnyEx() ? $"{r.Url}{CLIP_BRD__URL_REQ_HEAD_SEP_CHAR}{ExtensionRequestHeader.ToJson( r.RequestHeaders )}" : r.Url ) );
