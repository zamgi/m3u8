...
namespace m3u8.download.manager.controllers
{
    ...
    internal sealed class DownloadController : IDisposable
    {
        ...
        {
            [M(O.AggressiveInlining)] //for live-stream
        ...
        public DownloadController( DownloadListModel model, SettingsPropertyChangeController sc, m3u8_client_factory_enum_type m3u8_client_factory_type
            ...
            , m3u8_processor.ILogger logger = null )
        {
            ...
            _m3u8_client_factory_type = m3u8_client_factory_type;
            _m3u8_client_factory = m3u8_client_factory_maker.get( m3u8_client_factory_type );

            _CrossDownloadInstanceRestriction = new cross_download_instance_restriction( sc.MaxCrossDownloadInstance );
            _DownloadThreadsSemaphoreFactory  = new _download_threads_semaphore_factory_( sc.ShareMaxDownloadThreadsBetweenAllDownloadsInstance,
                                                                                          sc.MaxDegreeOfParallelism );
            _DownloadThreadsSemaphoreFactory_4_Parts = new _download_threads_semaphore_factory_( sc.ShareMaxDownloadThreadsBetweenAllDownloadsInstance,
                                                                                           sc.MaxDegreeOfParallelism );
        ...
        public void Dispose()
        {
            ...
            if ( _DownloadThreadsSemaphoreFactory_4_Parts != null )
            {
                _DownloadThreadsSemaphoreFactory_4_Parts.Dispose();
                _DownloadThreadsSemaphoreFactory_4_Parts = null;
            }
            ...
            , CancellationTokenSource cts = null )
        {
            using ( var mc = _m3u8_client_factory.Create( webProxy, requestTimeoutByPart, attemptRequestCountByPart: 1 ) )
            {
                try
                {
                    var m3u8File = await mc.DownloadFile( m3u8FileUrl, requestHeaders, cts?.Token ?? CancellationToken.None ).CAX();
        ...
        private async void SettingsController_PropertyChanged( Settings settings, string propertyName )
        {
            switch ( propertyName )
            {
                ...
                {
                    await ProcessChangedMaxDegreeOfParallelism( settings.MaxDegreeOfParallelism );                    
                }
                break;
                ...
                {
                    var v = settings.MaxCrossDownloadInstance;
                    _CrossDownloadInstanceRestriction.SetMaxCrossDownloadInstance( v );
                    ProcessCrossDownloadInstanceRestriction( v );
                }
                break;
        ...
        {
            var isDownloading = (_Dict.Count != 0);
            if ( !isDownloading )
            {
                _DownloadThreadsSemaphoreFactory.ReleaseAll();
                _DownloadThreadsSemaphoreFactory_4_Parts.ReleaseAll();
            }
            IsDownloadingChanged?.Invoke( isDownloading );
        }
        ...
        private async Task ProcessChangedMaxDegreeOfParallelism( int maxDegreeOfParallelism )
        {
            ...
            _DownloadThreadsSemaphoreFactory.ResetMaxDegreeOfParallelism( maxDegreeOfParallelism, ResetSemaphoreModeEnum.SetInitalCountAsCurrent );
            _DownloadThreadsSemaphoreFactory_4_Parts.ResetMaxDegreeOfParallelism( maxDegreeOfParallelism, ResetSemaphoreModeEnum.SetInitalCount2Max );

            _StreamPool.ChangeCapacity( maxDegreeOfParallelism );
            _RespBufPool.ChangeCapacity( maxDegreeOfParallelism );
            _TimeoutCtsPool.ChangeCapacity( maxDegreeOfParallelism );

            ph.ResetMaxDegreeOfParallelism_4_NotShareMaxDownloadThreadsBetweenAllDownloadsInstance_DownloadThreadsSemaphore( maxDegreeOfParallelism );
            ph.ContinueAll_Paused( tuples );
        }
        #endregion

        #region [.Process 'CrossDownloadInstanceRestriction'.]
        [M(O.AggressiveInlining)] private bool CanStartOrSetWaitCrossDownloadInstanceRestriction( DownloadRow row, bool checkActualRunningCount )
        {
            ...
            if ( maxCrossDownloadInstance.HasValue ) //if used cross download instance restriction
            {
                ...
                var runningCount = checkActualRunningCount ? _Model.GetRows_Enumerable().Count( row => row.IsRunning() ) : Volatile.Read( ref _RealRunningCount );
        ...
        private void ProcessCrossDownloadInstanceRestriction( int? maxCrossDownloadInstance )
        {
            ...
            try
            {
                ...
                void PausedExtraDownloads( int d )
                {
                    if ( d < 0 )
                    {
                ...
                if ( maxCrossDownloadInstance.HasValue ) //if used cross download instance restriction
                {
                    ...
                    var actualRunningCount = _Model.GetRows_Enumerable().Count( row => row.Status.IsRunningOrStarted() );
                    ...
                    {                        
                        ...
                        if ( 0 < d ) StartAdditionalDownloads( d );
                        return;
                    }
        ...
        //private static DownloadRow.DownloadStatusChangedDelegate _SetRowStatus2WaitAfterPauseDelegate = new DownloadRow.DownloadStatusChangedDelegate( SetRowStatus2WaitAfterPause );
        private static void SetRowStatus2WaitAfterPause( DownloadRow row, DownloadStatus prevStatus, DownloadStatus newStatus )
        {
            Debug.Assert( newStatus == DownloadStatus.Paused );

            row.OnDownloadStatusChanged -= SetRowStatus2WaitAfterPause; // _SetRowStatus2WaitAfterPauseDelegate;
            if ( newStatus == DownloadStatus.Paused ) row.StatusWait();
        }
        #endregion
        #endregion
        ...
        public async void Start( DownloadRow row )
        {
            ...
            if ( _Dict.TryGetValue( row, out var x ) )
            {
                ...
                {
                    if ( !CanStartOrSetWaitCrossDownloadInstanceRestriction( row, checkActualRunningCount: true ) )
                    {
                        return;
                    }
                    x.waitIfPausedEventWrapper.ResetNeedWait();
                }
                return;
            }
            #endregion
            ...
            if ( !CanStartOrSetWaitCrossDownloadInstanceRestriction( row, checkActualRunningCount: false ) )
            {
                return;
            }
            #endregion
            ...
            {
                await StartRoutine( row, t.m3u8FileUrl, deleteOutputFilesWhenCancelOrError: false );
            }
            #endregion
            ...
            #region [.cross download instance restriction.]
            ProcessCrossDownloadInstanceRestriction( _CrossDownloadInstanceRestriction.GetMaxCrossDownloadInstance() );
            #endregion
        }
        private async Task StartRoutine( DownloadRow row, Uri m3u8FileUrl, bool deleteOutputFilesWhenCancelOrError )
        {
            ...
            using ( var downloadThreadsSemaphore_4_Parts = _DownloadThreadsSemaphoreFactory_4_Parts.Get() )
            {
                ...
                try
                {
                    ...
                    var m3u8File = await mc.DownloadFile( m3u8FileUrl, row.RequestHeaders, cts.Token );
                    ...
                    var dpsr = await Task.Run( async () =>
                    {
                        ...
                        {
                            row.SetStatus( DownloadStatus.Running, row => !row.IsWait() );
                        ...
                        var waitingIfPausedBefore = new Action( () =>
                        {
                            //if ( !row.IsWait() ) row.SetStatus( DownloadStatus.Paused );
                            row.SetStatus( DownloadStatus.Paused, row => !row.IsWait() );
                        });
                        var waitingIfPausedAfter = new Action( () => row.SetStatus( DownloadStatus.Running ) );
                        var waitingIfPausedBefore_4_Parts = new Action< m3u8_part_ts >( part =>
                        {
                        ...
                        var waitingIfPausedAfter_4_Parts  = new Action< m3u8_part_ts >( part =>
                        {
                        ...
                        var ip = new m3u8_processor.DownloadPartsAndSaveInputParams()
                        {
                            ...
                            Logger                           = _Logger,
                            ReceivedAndWritedPartsProcessor  = _ReceivedAndWritedPartsProcessor,
        ...
        private async Task StartLiveStreamRoutine( DownloadRow row, Uri m3u8FileUrl )
        {
            ...
            switch ( _m3u8_client_factory_type )
            {
                ...
                case m3u8_client_factory_enum_type.HttpMessageInvoker:
                    ...
                    break;
                default:
                    throw (new ArgumentException( _m3u8_client_factory_type.ToString() ));
            }
            ...
            using ( var downloadThreadsSemaphore = _DownloadThreadsSemaphoreFactory.Get() )
            {
                var tup = Tuple.Create( cts, waitIfPausedEventWrapper, downloadThreadsSemaphore, _Dict.Count );
                _Dict.Add( row, tup, DisposeExistsTupleWhenAdd2Dict ); Fire_IsDownloadingChanged();
                ...
                try
                {
                    ...
                    await Task.Run( async () =>
                    {
                        ...
                        var p = new i_m3u8_live_stream_downloader.InitParams()
                        {
                            ...
                            WaitIfPausedHolder = new WaitIfPausedHolder( waitIfPausedEventWrapper, () => row.SetStatus( DownloadStatus.Paused ), null ),
                            ThrottlerBySpeed   = _ThrottlerBySpeed,
                            TimeoutCtsPool     = _TimeoutCtsPool,
        ...
        #region [.PausedHelper.]
        private readonly struct PausedHelper
        {
            /// <summary>
            /// 
            /// </summary>
            ...
            #region [.field's.]
            private readonly ConcurrentDictionary< DownloadRow, DownloadController.Tuple > _Dict;
            ...
            private static Result/*async Task< Result >*/ PausedAll_ByStatus_GetThem( ConcurrentDictionary< DownloadRow, DownloadController.Tuple > dict, params DownloadStatus[] statuses )
            {
                ...
                #region comm. parallel.
                /*
                var coll = (IReadOnlyCollection< KeyValuePair< DownloadRow, DownloadController.Tuple > >) dict;
                await coll.ForEachAsync( Math.Min( 10, dict.Count ), CancellationToken.None, (p, ct) =>
                {
                    var row = p.Key;
            ...
            public void ResetMaxDegreeOfParallelism_4_NotShareMaxDownloadThreadsBetweenAllDownloadsInstance_DownloadThreadsSemaphore( int maxDegreeOfParallelism )
            {
                foreach ( var t in _Dict.Values )
                {
                    if ( !t.downloadThreadsSemaphore.ShareMaxDownloadThreadsBetweenAllDownloadsInstance )
                    {
                        t.downloadThreadsSemaphore.ResetSemaphore( maxDegreeOfParallelism, ResetSemaphoreModeEnum.SetInitalCountAsCurrent );
                    }

                    if ( (t.downloadThreadsSemaphore_4_Parts != null) && !t.downloadThreadsSemaphore_4_Parts.ShareMaxDownloadThreadsBetweenAllDownloadsInstance )
                    {
                        t.downloadThreadsSemaphore_4_Parts.ResetSemaphore( maxDegreeOfParallelism, ResetSemaphoreModeEnum.SetInitalCount2Max );
                    }
                }
            }
            #endregion
            ...
            Action< DownloadRow > afterSuccesDeleteAction )
        {
            foreach ( var row in rows )
            {
                using ( var statusTran = this.CancelIfInProgress_WithTransaction( row ) )
                {
                    await deleteFilesAction( row, ct ).CAX();
                    statusTran.CommitStatus();
        ...
        public bool TryChangeSettings( DownloadRow row, in web_proxy_info webProxyInfo, TimeSpan? timeout, int? attemptRequestCount )
        {
            if ( (row != null) && _Dict.TryGetValue( row, out var t ) )
            {
                ...
                var csp = new i_m3u8_client.ChangeSettingsParams()
                {
    ...
    internal static class DownloadControllerExtensions
    {
        ...
        public static void StatusFinished( this DownloadRow row, in m3u8_processor.DownloadPartsAndSaveResult dpsr, TimeSpan elapsed )
        {
