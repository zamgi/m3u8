    ...
    internal static class DownloadControllerExtensions
    {
        public static void Output( this LogListModel log, in m3u8_file_t m3u8File )
        {
            ...
            log.BeginUpdate();
            {
                ...
                {
                    log.AddRequestRowSuccess( line );
                }
                log.AddRequestRowFinish( "------------------------" );
                log.AddRequestRowFinish( $" patrs count: {m3u8File.Parts.Count}" );
                log.AddRequestRowFinish( "------------------------" );
            }
            log.EndUpdate();
        }
        ...
        public static void StatusFinished( this DownloadRow row, in _m3u8_processor_.DownloadPartsAndSaveResult dpsr, TimeSpan elapsed )
        {
            ...
            var log = row.Log;
            log.AddEmptyRow();
            log.AddRequestRowFinish( $" downloaded & writed parts {dpsr.PartsSuccessCount} of {dpsr.TotalParts}" );
            log.AddEmptyRow();
            log.AddRequestRowFinish( $" elapsed: {elapsed}" );
            log.AddRequestRowFinish( $"         file: '{dpsr.OutputFileName}'" );
            log.AddRequestRowFinish( $"       size: {(dpsr.TotalBytes >> 20).ToString( "0,0" )} mb" );
        }
