...
namespace m3u8.infrastructure
{
    ...
    internal static class HttpClientFactory
    {
        ...
        private static Dictionary< TimeSpan, HttpClient > _Dict;

        static HttpClientFactory() => _Dict = new Dictionary< TimeSpan, HttpClient >();

        public static HttpClient Get( in init_params ip ) => Get( ip.Timeout );
        public static HttpClient Get( TimeSpan? timeout = null )
        {
            var key = timeout.GetValueOrDefault( TimeSpan.Zero );
            lock ( _Dict )
            {
                if ( !_Dict.TryGetValue( key, out var hc ) )
                {
                    hc = new HttpClient();
                    if ( timeout.HasValue )
                    {
...
namespace m3u8
{
    ...
    {
        /*public static m3u8_client Create() => Create( HttpClientFactory.Get() );
